...
fn drop_open() {
    tokio_uring_xitca::start(async {
        let tempfile = tempfile();
        drop(File::create(tempfile.path()));
...
#[ignore]
fn drop_off_runtime() {
    let file = tokio_uring_xitca::start(async {
        let tempfile = tempfile();
...
fn rename() {
    ...
    tokio_uring_xitca::start(async {
        ...
        let mut new_file_name = old_path.file_name().unwrap_or_else(|| OsStr::new("")).to_os_string();
        new_file_name.push("_renamed");

        let new_path = old_path.with_file_name(new_file_name);

        tokio_uring_xitca::fs::rename(&old_path, &new_path).await.unwrap();
...
fn basic_fallocate() {
    tokio_uring_xitca::start(async {
        ...
        file.fallocate(0, 1024, libc::FALLOC_FL_ZERO_RANGE).await.unwrap();
        file.sync_all().await.unwrap();
        ...
        file.fallocate(0, 2048, libc::FALLOC_FL_ZERO_RANGE | libc::FALLOC_FL_KEEP_SIZE)
            .await
            .unwrap();
        file.sync_all().await.unwrap();

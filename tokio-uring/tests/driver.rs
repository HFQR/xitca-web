...
use tokio_uring_xitca::{buf::IoBuf, fs::File};
...
fn complete_ops_on_drop() {
    ...
    unsafe impl tokio_uring_xitca::buf::IoBufMut for MyBuf {
...
fn too_many_submissions() {
    ...
    tokio_uring_xitca::start(async {
        ...
        for _ in 0..600 {
            poll_once(async {
                file.write_at(b"hello world".to_vec(), 0).submit().await.0.unwrap();
            })
            .await;
        }
    });
...
fn completion_overflow() {
    ...
    tokio_uring_xitca::builder()
        .entries(squeue_entries)
        .uring_builder(tokio_uring_xitca::uring_builder().setup_cqsize(cqueue_entries))
        .start(async move {
            let mut js = JoinSet::new();

            for _ in 0..spawn_cnt {
                js.spawn_local(tokio_uring_xitca::no_op());
            }

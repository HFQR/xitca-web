...
impl<T: BoundedBuf> Op<SendTo<T>> {
    pub(crate) fn send_to(fd: &SharedFd, buf: T, socket_addr: Option<SocketAddr>) -> io::Result<Op<SendTo<T>>> {
        ...
        CONTEXT.with(|x| {
            x.handle().expect("Not in a runtime context").submit_op(
                ...
                |send_to| {
                    opcode::SendMsg::new(types::Fd(send_to.fd.raw_fd()), send_to.msghdr.as_ref() as *const _).build()
                },
            )
        })
    }

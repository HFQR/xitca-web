...
impl<T: BoundedBuf> Op<Writev<T>> {
    pub(crate) fn writev_at(fd: &SharedFd, mut bufs: Vec<T>, offset: u64) -> io::Result<Op<Writev<T>>> {
        ...
        CONTEXT.with(|x| {
            x.handle().expect("Not in a runtime context").submit_op(
                ...
                |write| {
                    opcode::Writev::new(types::Fd(fd.raw_fd()), write.iovs.as_ptr(), write.iovs.len() as u32)
                        .offset(offset as _)
                        .build()
                },
            )

...
impl Handle {
    ...
    pub(crate) fn register_buffers(&self, buffers: Rc<RefCell<dyn FixedBuffers>>) -> io::Result<()> {
        self.inner.borrow_mut().register_buffers(buffers)
    }

    pub(crate) fn unregister_buffers(&self, buffers: Rc<RefCell<dyn FixedBuffers>>) -> io::Result<()> {
    ...
    pub(crate) fn poll_multishot_op<T>(&self, op: &mut Op<T, MultiCQEFuture>, cx: &mut Context<'_>) -> Poll<T::Output>
    where

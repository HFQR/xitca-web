...
fn main() {
    ...
    tokio_uring_xitca::start(accept_loop(socket_addr));
}
...
async fn accept_loop(listen_addr: SocketAddr) {
    ...
    loop {
        ...
        tokio_uring_xitca::spawn(echo_handler(stream, peer, registry.clone()));
    }
}
...
async fn echo_handler<T: IoBufMut>(stream: TcpStream, peer: SocketAddr, registry: FixedBufRegistry<T>) {

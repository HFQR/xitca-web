...
async fn main1() -> io::Result<()> {
    ...
    if unexpected == 0 {
        ...
        Err(io::Error::other(format!("{unexpected} unexpected result(s)")))
    }
}

async fn statx<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let _statx = tokio_uring_xitca::fs::statx(path).await?;
    Ok(())
}

async fn statx_builder<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let _statx = tokio_uring_xitca::fs::StatxBuilder::new()
        .pathname(path)?
        .statx()
        .await?;
    Ok(())
...
async fn statx_builder2<P: AsRef<Path>>(dir_path: P, rel_path: P) -> io::Result<()> {
    ...
    // descriptor.
    let f = tokio_uring_xitca::fs::File::open(dir_path).await?;
...
async fn matches_mode<P: AsRef<Path>>(path: P, want_mode: u16) -> io::Result<()> {
    let statx = tokio_uring_xitca::fs::StatxBuilder::new()
...
async fn touch_file<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let file = tokio_uring_xitca::fs::OpenOptions::new()
        .append(true)
        .create(true)
        .open(path)
        .await?;
...
async fn is_regfile<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let (_is_dir, is_regfile) = tokio_uring_xitca::fs::is_dir_regfile(path).await;

    if is_regfile {
        Ok(())
    } else {
        Err(io::Error::other("not regular file"))
    }
}

async fn is_dir<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let (is_dir, _is_regfile) = tokio_uring_xitca::fs::is_dir_regfile(path).await;

    if is_dir {
        Ok(())
    } else {
        Err(io::Error::other("not directory"))
    }
}

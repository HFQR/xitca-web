...
macro_rules! match_tests {
    ($($name:ident {
        ...
        fn $name() {
            ...
            for route in $routes {
                router.insert(route, RefCell::new(route.to_owned()))
            ...
            $(match router.at($path) {
                ...
                Ok(result) => {
                    $($( @$some )?
                        if &*result.value.borrow() != $route {
                            panic!(
                                "Wrong value for route '{}'. Expected '{}', found '{}')",
                                $path, &*result.value.borrow(), $route
                            );
                        }

                        let expected_params = vec![$((BytesStr::from($key), Box::from($val))),*];
                        ...
                        router.at($path).unwrap().value.borrow_mut().push_str("CHECKED");
                        assert!(router.at($path).unwrap().value.borrow().contains("CHECKED"));

                        let mut val = router.at($path).unwrap().value.borrow_mut();
                    ...
                    $($( @$none )?
                        panic!(
                            ...
                            $path,
                            result.params.into_iter().collect::<Vec<_>>()
                        );
                    )?
                }
            })*

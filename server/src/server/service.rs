...
pub type ServiceObj = Box<
    dyn for<'a> xitca_service::object::ServiceObject<
            (&'a str, &'a [(String, ListenerDyn)], Arc<ShutdownToken>),
...
impl<'a, F, Req> Service<(&'a str, &'a [(String, ListenerDyn)], Arc<ShutdownToken>)> for Container<F, Req>
where
    ...
    Req: TryFrom<Stream> + 'static,
{
    ...
    async fn call(
        &self,
        (name, listeners, st): (&'a str, &'a [(String, ListenerDyn)], Arc<ShutdownToken>),
        ...
        let handles = listeners
            .iter()
            .filter(|(n, _)| n == name)
            .map(|(_, listener)| worker::start(listener, &service, st.clone()))
            .collect::<Vec<_>>();
...
where
    ...
    Req: TryFrom<Stream> + 'static,
{
    type Service: ReadyService + Service<(Req, Arc<ShutdownToken>)>;

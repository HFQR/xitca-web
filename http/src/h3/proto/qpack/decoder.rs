...
impl Decoder {
    ...
    pub fn on_encoder_recv<R: Buf, W: BufMut>(&mut self, read: &mut R, write: &mut W) -> Result<usize, DecoderError> {
    ...
    fn parse_header_field<R: Buf>(table: &DynamicTableDecoder, buf: &mut R) -> Result<HeaderField, DecoderError> {
        let first = buf.chunk()[0];
...
mod tests {
    use super::super::{
        encode_stateless,
        tests::helpers::{TABLE_SIZE, build_table_with_size},
    };
    use super::*;
    ...
    fn test_insert_field_with_wrong_name_index_from_dynamic_table() {
        ...
        assert_eq!(
            res,
            Err(DecoderError::DynamicTable(DynamicTableError::BadRelativeIndex(3000)))
        );

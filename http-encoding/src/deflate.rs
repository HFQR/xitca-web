...
pub(super) use flate2::write::DeflateDecoder;

impl<Item> AsyncDecode<Item> for DeflateDecoder<Writer>
where
...
fn decode(mut this: DeflateDecoder<Writer>, buf: &[u8]) -> io::Result<(DeflateDecoder<Writer>, Option<Bytes>)> {
    this.write_all(buf)?;
    this.flush()?;

...
pub(super) use flate2::write::GzDecoder;

impl<Item> AsyncDecode<Item> for GzDecoder<Writer>
where
...
fn decode(mut this: GzDecoder<Writer>, buf: &[u8]) -> io::Result<(GzDecoder<Writer>, Option<Bytes>)> {
    this.write_all(buf)?;
    this.flush()?;

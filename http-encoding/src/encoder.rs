    ...
    Bytes: From<Item>,
{
    ...
    fn code(self, item: Item) -> io::Result<(Self, Option<Self::Item>)> {
        match self.encoder {
            _ContentEncoder::Identity(encoder) => {
                <IdentityCoder as AsyncCode<Item>>::code(encoder, item).map(|(encoder, item)| (encoder.into(), item))
            }
            #[cfg(feature = "br")]
    ...
    fn code_async(self, item: Item) -> Self::Future {
        async move {
            match self.encoder {
                _ContentEncoder::Identity(encoder) => <IdentityCoder as AsyncCode<Item>>::code_async(encoder, item)
                    .await
                    .map(|(encoder, item)| (encoder.into(), item)),
                #[cfg(feature = "br")]
                _ContentEncoder::Br(encoder) => <BrotliEncoder<Writer> as AsyncCode<Item>>::code_async(encoder, item)
                    .await
                    .map(|(encoder, item)| (encoder.into(), item)),
                #[cfg(feature = "gz")]
                _ContentEncoder::Gz(encoder) => <GzEncoder<Writer> as AsyncCode<Item>>::code_async(encoder, item)
                    .await
                    .map(|(encoder, item)| (encoder.into(), item)),
                #[cfg(feature = "de")]
                _ContentEncoder::De(encoder) => <DeflateEncoder<Writer> as AsyncCode<Item>>::code_async(encoder, item)
                    .await
                    .map(|(encoder, item)| (encoder.into(), item)),
            }
        }
